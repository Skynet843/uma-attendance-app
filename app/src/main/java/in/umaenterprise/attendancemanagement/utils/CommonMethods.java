...
import in.umaenterprise.attendancemanagement.activity.SignInActivity;
...
import in.umaenterprise.attendancemanagement.service.SendFCMNotificationService;
...
//							android.provider.Settings.System.AUTO_TIME, 1);
    ...
    public static double[] getDayTypeForMonthWise(ShopTimingModel timeModel,
                                                  String strSelectedPunchInTime, String strSelectedPunchOutTime,boolean isShift2,int overTime) {

        Log.d("CALCULATE", "getDayTypeForMonthWise: "+overTime);
        double[] doubles = new double[2];
        int totalWorkingMinutes = CommonMethods.calculateTotalHours(strSelectedPunchInTime,
                strSelectedPunchOutTime);
        totalWorkingMinutes+=overTime;
        ...
        Date hourForFullShiftInDate = null;
        try {

            hourForFullShiftInDate = actTimeFormat.parse(isShift2?timeModel.getHoursForFullDayS2():timeModel.getHoursForFullDay());
        } catch (ParseException e) {
            e.printStackTrace();
        }
        ...
        try {
            ...
            String[] fh=timeModel.getHoursForFullDay().split(":");
            String[] fh2=timeModel.getHoursForFullDayS2().split(":");

            int hour=Integer.parseInt(fh[0]);
            int min=Integer.parseInt(fh[1]);
            int hour2=Integer.parseInt(fh2[0]);
            int min2=Integer.parseInt(fh2[1]);

            int thour=(min+min2)/60;
            int tmin=(min+min2)%60;
            thour+=hour+hour2;

            String thourString=thour+"";
            if(thour/10==0){
                thourString="0"+thourString;
            }
            String tminString=tmin+"";
            if(tmin/10==0){
                tminString="0"+tminString;
            }
            hourForFullDayInDate = actTimeFormat.parse(thourString+":"+tminString);
            Log.d("SOUVIK", "getDayTypeForMonthWise: "+hourForFullDayInDate);
        } catch (ParseException e) {
            e.printStackTrace();
        }


        Log.d("SOUVIK", "getDayTypeForMonthWise: "+hourForFullDayInDate.toString());
        ...
        } else if (totalWorkingHoursInDate.compareTo(hourForFullShiftInDate) == 0
                || totalWorkingHoursInDate.compareTo(hourForFullShiftInDate) > 0) {
            doubles[0] = 0.5;
            if (totalWorkingHoursInDate.compareTo(hourForFullShiftInDate) > 0) {
                long diff = totalWorkingHoursInDate.getTime() - hourForFullShiftInDate.getTime();
                int hours = (int) (diff / (1000 * 60 * 60));
                int mins = (int) (diff / (1000 * 60)) % 60;
                doubles[1] = (hours * 60) + mins;
            } else {
        ...
        AsyncTask.execute(new Runnable() {
            ...
            public void run() {
                Collections.sort(mTransactionList, new Comparator<AttendanceModel>() {
                    @Override
                    public int compare(AttendanceModel p1, AttendanceModel p2) {
                        ...
                        for (int i = 0; i < length; i++) {
                            ...
                            if (model.getType().equalsIgnoreCase(activity.getString(R.string.label_public_holiday))
                                ...
                                if (model.getPresentDay() == -1) {//Means user has not do punch out
                                    ...
                                    timeDuration = model.getPunchInTime() + " - " +
                                            model.getPunchOutTime()+"\n"+model.getPunchInTimeS2()+" - "+model.getPunchOutTimeS2();
                                }
